...
onMounted(() => {
  ...
  _chart.subscribeCrosshairMove((param) => {
    ...
    if (
      ...
      const _fid = param.seriesData.get(areaSeries)!
      const fid = _fid.value !== undefined ? _fid.value : _fid.time
  ...
  <div ref="container" class="w-full h-full">
    ...
    <div class="tooltip">
      <div v-if="tooltipData.time" class="dark:text-gray-200 text-gray-600 text-xs">
        ...
        <div v-if="tooltipData.fid > 0" class="font-bold" :class="tooltipData.fid >= 300 ? 'text-red-500' : tooltipData.fid >= 100 ? 'text-yellow-500' : 'text-green-500'">
          FID {{ useHumanMs(tooltipData.fid) }}
        </div>
