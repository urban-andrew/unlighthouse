...
onMounted(() => {
  ...
  _chart.subscribeCrosshairMove((param) => {
    ...
    if (
      ...
      const _ttfb = param.seriesData.get(areaSeries)!
      const ttfb = _ttfb.value !== undefined ? _ttfb.value : _ttfb.time
  ...
  <div ref="container" class="w-full h-full">
    ...
    <div class="tooltip">
      <div v-if="tooltipData.time" class="dark:text-gray-200 text-gray-600 text-xs">
        ...
        <div class="font-bold" :class="tooltipData.ttfb >= 1800 ? 'text-red-500' : tooltipData.ttfb >= 800 ? 'text-yellow-500' : 'text-green-500'">
          TTFB {{ useHumanMs(tooltipData.ttfb) }}
        </div>
      </div>
    </div>
  </div>
